...
本版本围绕**更稳定的倍速体验**与**更顺手的设置面板**两大主题展开:重构长按 `→` 加速并复用 B 站原生提示 UI,修复全屏按钮显示与多集视频切集后倍速被重置的问题,将内容脚本与设置面板全面模块化,为设置面板加入「当前默认倍速」展示与自动保存,并把 e2e 测试重整为按功能拆分的套件。

#### ✨ 功能与特性更新

- **长按 `→` 加速**:由扩展在捕获阶段接管右方向键长按手势,倍速提升至当前倍速的 **2 倍**,并复用播放器原生提示样式与 lottie 箭头动画。
- **默认倍速可视化**:设置面板顶部新增「当前默认倍速」信息卡,实时跟随 `chrome.storage.sync` 中记忆的倍速。
- **设置自动保存**:输入框失焦、输入框内按 Enter、勾选切换、删除 / 步进,以及标题规则拖拽 / 键盘排序后均自动落盘,无需再点「保存设置」(按钮保留为手动兜底)。
- **切集倍速稳定**:多集视频(分 P)切集 / 切换清晰度时,不再被 `video.load()` 触发的程序性 `ratechange` 覆盖期望倍速。
- **全屏按钮修复**:预制倍速按钮在全屏状态下正常显示。
- **工程化**:内容脚本拆分为 7 个模块,设置面板改用 ES Modules;e2e 重整为统一入口 + 按功能拆分的规格,共 **123** 项断言。

#### 📝 提交明细

##### `00ff141` — fix(ui): 预制倍速按钮在全屏状态下正常显示

- `.bprft-chip` 增加 flex 布局并水平、垂直居中,修复全屏状态下预制倍速按钮显示异常的问题。

##### `95ce11e` — feat: 长按→方向键倍速由扩展接管并复用原生提示 UI

- 在 `window` 捕获阶段劫持 `ArrowRight` 长按手势,倍速设为当前倍速的 **2 倍**。
- 按原生结构创建 `.bpx-player-three-playrate-hint-loop` 提示,复用播放器自带样式;左侧图标还原原生 lottie 三段箭头动画(透明度 15%↔80%,相位差 1/6 秒)。
- 轮播仅保留当前倍速并垂直居中,移除上下相邻的半透明预设。
- 短按保留快进 5 秒;失焦 / 页面隐藏自动取消加速。
- 修正 B 站 1x 时把按钮渲染为「倍速」而非数值的问题。
- 移除会把临时倍速写入 `desired` / `defaultRate` 的 result `MutationObserver`,并让 `reapply` 不再监听 `ratechange`,避免倍速被固定且不写回默认值。
- `manifest` 改为 `document_start`,确保监听早于 B 站脚本注册。

##### `056d808` — refactor: 项目模块化拆分并整理目录结构

- 目录整理:图标 → `icons/`,内容脚本与样式 → `src/content/`,设置面板 → `src/popup/`。
- 内容脚本拆分为 7 个模块(config / utils / video / ui / title / boost / main),按 manifest 顺序注入并共享 `window.BPRFT` 命名空间。
- 设置面板改用 ES Modules,拆分为 i18n / dom / config / storage / presets / rules / stepper / panels。
- 更新 `manifest.json`、`popup.html` 与 README 中的资源及脚本路径。
- e2e 新增 popup 冒烟测试(模块加载、i18n、预设渲染、图标解析、无报错);行为无变化。

##### `c5fc240` — fix(video): 切集后重新应用倍速并重整 e2e 测试套件

- **修复**:多集视频(分 P)切集 / 切换清晰度后倍速失效。B 站会调用 `video.load()`,按规范将 `playbackRate` 重置为 `defaultPlaybackRate`(通常为 1)并触发 `ratechange`,扩展此前误将这次程序性重置当作用户意图,覆盖了期望倍速。现在切源期间忽略该 `ratechange` 并自动重新应用期望倍速,并以 `currentSrc` 变化兜底;外部真实改速(切源窗口之外)仍会被采纳,倍速按钮文字始终与期望值保持一致。
- **测试**:`tools/e2e.mjs` 改为统一入口,按功能拆分为 `tools/e2e/specs/` 下的 popup / 核心调速 / 标题匹配 / 长按加速 / 切集 / 跨视频标签匹配 / 持久化;新增多级标签匹配与跨视频(非多 P)切换回归,标题含「歌」「周杰伦」及两者兼有时按优先级命中,覆盖整页跳转与站内 SPA 推荐位切换。
- **文档**:README 增补变更日志与文件结构说明。

##### `523f986` — feat(popup): 展示默认倍速并支持失焦/Enter/排序自动保存

- 设置面板顶部新增「当前默认倍速」信息卡,读取并实时跟随 `chrome.storage.sync` 的 `rate`。
- 新增 `autosave` 模块:输入框失焦、按 Enter、勾选切换、步进 / 删除操作后自动保存,无需点「保存设置」。
- 标题规则拖拽 / 键盘调整优先级后自动保存(rules 通过自定义事件通知,避免与 storage 循环依赖)。
- 保存逻辑增加快照去重与防抖,重置前取消挂起保存;自动保存不重排 DOM,避免打断正在进行的编辑。
- 保留手动「保存设置」按钮作为兜底,「自动添加当前视频」后立即落盘。
- e2e 新增默认倍速展示、失焦 / Enter、拖拽 / 键盘排序自动保存断言(popup 规格 35 项)。
